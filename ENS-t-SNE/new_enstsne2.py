...
class ENSTSNE():
    ...
    def gd(self,maxiter,momentum=0.9,lr=100,tol=1e-5,alpha=0.001,beta1 = 0.99, beta2 = 0.999):
        X = self.X 
        P = self.projections
        ...
        for i in tqdm(range(maxiter)):
            ...
            step = alpha * mhat / (np.sqrt(vhat) + tol)

            X -= step + momentum * change
            if not fixed: 
...
if __name__ == "__main__":
    ...
    dists, labels, X = load_clusters(n=200, dims=[3,3,3], n_clusters=[4,3,2])
    ...
    for alpha in alphas:
        for beta1 in beta1s:
            for beta2 in beta2s: 
                ...
                hist = enstsne.gd(2000,alpha=alpha,beta1=beta1,beta2=beta2)
                from eval import vis_3d
                # vis_2d(enstsne.get_images(),labels,"test")
                vis_3d(enstsne.get_embedding(), labels, 
                    f"adam_test/a={alpha}b1={beta1}b2={beta2}_3d.png")

                plt.clf()
                plt.plot(range(len(hist[1:])), hist[1:])
                plt.savefig(f"adam_test/a={alpha}b1={beta1}b2={beta2}_plot.png")
                plt.clf()

                vis_2d(enstsne.get_images(), labels, f"adam_test/a={alpha}b1={beta1}b2={beta2}_2d.png")
                plt.clf()